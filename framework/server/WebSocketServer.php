...
class WebSocketServer extends HttpServer
{
    ...
    private function addSubscribeProcess($server)
    {
        if ($this->getValueFromConf('distributed')) {
            $this->getProcessManager();
            $auto =new WebsocketSubscribeProcess($server);
    ...
    protected function afterStart(\swoole_server $server)
    {
        if ($this->getMode() == SWOOLE_BASE) {
            $this->addSubscribeProcess($server);
        }
    ...
    protected function onHandShake()
    {
        $this->_server->on('handshake', function (\swoole_http_request $request, \swoole_http_response $response)
        {
            ...
            if ($this->getValueFromConf('distributed')) {
                // 记录节点
                $container->getComponent(\getModule(), 'redis')->set('ws_distribute_node:fd:' . $request->fd, getMacAddr());
            }
    ...
    protected function onMessage()
    {
        $this->_server->on('message', function (\swoole_websocket_server $server, $frame)
        {
            if ($frame->data == WEBSOCKET_PING) {
                $server->push($frame->fd, WEBSOCKET_PONG);
                return false;
            }
...
//            目前不支持过大消息和二进制数据
            ...
            $action[0] = ucfirst(strtolower($action[0]));
    ...
    protected function afterClose(\swoole_server $server, int $fd, int $reactorId)
    {
        ...
        $container = Container::getInstance();
        if ($this->getValueFromConf('distributed')) {
            $container->getComponent($FD_SYSTEM[$fd], 'redis')->rm('ws_distribute_node:fd:' . $fd);
        }
